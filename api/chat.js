...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!GEMINI_API_KEY) {
            return res.status(500).json({ error: 'API key not configured', debug: 'No GEMINI_API_KEY env var' });
        }
...
Previous work (prototypes): tiskalagunarestaurant.web.app, bonanazarestourant.vercel.app, strandbekavar.web.app
...
Odgovaraj kratko i prijateljski na srpskom. Ako nešto ne znaš, uputi korisnika na kontakt.`
        ...
        console.error('Error calling Gemini:', error.message, error.stack);
        return res.status(500).json({ error: 'Failed to get AI response', message: error.message });
    }
}
